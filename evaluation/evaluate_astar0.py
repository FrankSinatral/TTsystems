...
def main():
    ...
    with open("10task_new_list.pkl", "rb") as f:
        task_list = pickle.load(f)
    for j in range(21, len(task_list)):
    ...
    # task_list = Parallel(n_jobs=-1)(delayed(process_task)(j, env, planner_config) for j in range(1000))

    # # Remove None values from the list
    # task_list = [task for task in task_list if task is not None]
    # with open("10task_new_list.pkl", "wb") as f:

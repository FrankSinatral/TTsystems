...
class TractorTrailerReachingEnv(Env):
    ...
    def run_simulation(self, save_dir=None):
        ...
        if not save_dir:
            if not os.path.exists("./rl_training/gif/tt_reaching"):
                os.makedirs("./rl_training/gif/tt_reaching")
                
            base_path = "./rl_training/gif/tt_reaching/path_simulation"
            extension = ".gif"
            
            all_files = os.listdir("./rl_training/gif/tt_reaching")
...
#              self.config["act_limit"]], 
#             dtype=np.float32)
#         continuous_action_space = spaces.Box(low=action_low, high=action_high)
#         discrete_action_space = spaces.Discrete(10)
#         self.action_space = spaces.Tuple((continuous_action_space, discrete_action_space))
#         # self.action_space = spaces.Box(low=action_low, high=action_high)
...
#     # implement tt system here
#     def step(self, action):
#         continous_action, n_step = action
        
#         old_state = self.controlled_vehicle.observe()
#         # clip action
#         action_clipped = np.clip(continous_action, -self.act_limit, self.act_limit)

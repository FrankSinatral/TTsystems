...
def generate_local_course(L, lengths, mode, maxc, step_size):
    ...
    for m, l, i in zip(mode, lengths, range(len(mode))):
        ...
        if m == "S":
            ...
            # steer = md.C_three_trailer.MAX_STEER
            steer = 0.6
        else:
            # steer = -md.C_three_trailer.MAX_STEER
            steer = -0.6
...
def extract_rs_control_list(rspath, maxc, step_size):
    """
    Extract all the control from rspath
    
    Inputs:
    rspath: rspath with path.L/ path.lengths/ path.ctypes property
    maxc: max steer
    step_size: every step size
    """
    L = rspath.L
    lengths = rspath.lengths
    mode = rspath.ctypes
    rs_control_list = []
    point_num = int(L / step_size) + len(lengths) + 3
    ...
    for m, l, i in zip(mode, lengths, range(len(mode))):
        ...
        while abs(pd) <= abs(l):
            ...
            px, py, pyaw, directions = \
                interpolate(ind, pd, m, maxc, ox, oy, oyaw, px, py, pyaw, directions)
            rs_control_list.append(np.array([d, steer]))
            pd += d
...
def pi_2_pi(theta):
    """
    returns an angle ranges from [-pi, pi]
    """
    while theta >= PI:
        theta -= 2.0 * PI
